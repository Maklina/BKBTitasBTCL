...
{
    public partial class NonMeterBillDetails : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                rvBrWiseNonMDetail.ProcessingMode = ProcessingMode.Local;
                ...
                {
                    rvBrWiseNonMDetail.LocalReport.ReportPath = Server.MapPath("~/ReportFile/RDLCFile/BrWiseNonMDetail.rdlc");
                }
                else
                {
                    rvBrWiseNonMDetail.LocalReport.ReportPath = Server.MapPath("~/ReportFile/RDLCFile/ZoneWiseNonMDetail.rdlc");
                }
                ...
                dtBrWiseNonMDetail = GetData("EXEC dbo.[BranchWiseNonMeterDetails] @dtFrom='" + dtFrom + "',@dtTo='" + dtTo + "',@branch='" + branchCode + "',@userRole='" + userRole + "'");

                if (dtBrWiseNonMDetail.Rows.Count > 0)
                {
                    this.rvBrWiseNonMDetail.LocalReport.SetParameters(new ReportParameter[] { p1, p2 });
                    ReportDataSource dtReport = new ReportDataSource("BrWiseNonMDetail", dtBrWiseNonMDetail);
                    rvBrWiseNonMDetail.LocalReport.DataSources.Clear();
                    rvBrWiseNonMDetail.LocalReport.DataSources.Add(dtReport);
                    rvBrWiseNonMDetail.LocalReport.Refresh();
                }
            }
        }
